...
import { OwnedContentGate } from '@/features/access/components/owned-content-gate';
...
import {
  canPurchaseBookFormat,
  hasBookReaderAccess,
  ownsEbook,
  ownsPhysical,
} from '@/features/account/services/ownership.service';
...
const FORMAT_LABELS: Record<BookPurchaseFormat, { en: string; ar: string }> = {
  ...
  bundle: { en: 'E-book + printed copy', ar: 'النسختان معاً' },
};
...
export function BookDetailPage() {
  ...
  if (!book) {
    return (
      <div className="min-h-screen bg-slate-50 p-16 text-center text-slate-700">
        {language === 'ar' ? 'الكتاب غير متاح.' : 'Book is unavailable.'}
      </div>
    );
  }

  const userId = user?.id;
  const hasEbook = userId ? ownsEbook(userId, book.id) : false;
  const hasPhysical = userId ? ownsPhysical(userId, book.id) : false;
  const canRead = userId ? hasBookReaderAccess(userId, book.id) : false;
  const related = getRelatedBooks(book, 3);
  const availableFormats = getAvailableFormats(book.id);

  const formats = availableFormats.map((format) => {
    const canPurchase = userId ? canPurchaseBookFormat(userId, book.id, format) : true;
    const owned = format === 'ebook' ? hasEbook : format === 'physical' ? hasPhysical : hasEbook && hasPhysical;
    let reason: string | undefined;
    if (owned) {
      reason =
        format === 'ebook'
          ? language === 'ar'
            ? 'تمتلك النسخة الإلكترونية'
            : 'E-book owned'
          : format === 'physical'
            ? language === 'ar'
              ? 'تمتلك النسخة المطبوعة'
              : 'Printed copy owned'
            : language === 'ar'
              ? 'تمتلك الباقة'
              : 'Bundle owned';
    } else if (!canPurchase) {
      reason = language === 'ar' ? 'هذه الصيغة غير متاحة للشراء' : 'This format is not available for purchase';
    }
    const price = getBookPrice(book, format);
    return {
      id: format,
      label: localize(FORMAT_LABELS[format]),
      price: price ?? 0,
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {hasEbook && (
              <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">
                {language === 'ar' ? 'نسخة إلكترونية' : 'E-book'}
              </span>
            )}
            {hasPhysical && (
              <span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-semibold text-slate-800">
                {language === 'ar' ? 'نسخة مطبوعة' : 'Printed'}
              </span>
            )}
          </div>
          <div className="mt-6 flex items-center gap-3">
            <ShareButton title={localize(book.title)} />
            <FavoriteButton
              favorite={{ itemId: book.id, kind: 'book', title: localize(book.title), href: `/books/${book.id}`, isAvailable: true }}
        ...
        <div className="space-y-6">
          <article className="rounded-lg border border-slate-200 bg-white p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {book.topics.map((topic) => (
                <span key={topic.en} className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">
                  {localize(topic)}
                </span>
              ))}
            </div>
          ...
          <article className="rounded-lg border border-slate-200 bg-white p-6">
            <h2 className="text-xl font-bold text-slate-950">
              {language === 'ar' ? 'اختيار صيغة الشراء' : 'Choose purchase format'}
            </h2>
                ...
                <div key={format.id} className="rounded-md border border-slate-200 p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      ...
                      {format.reason && !format.owned && (
                        <p className="mt-1 text-sm text-amber-700">{format.reason}</p>
                      )}
                    </div>
                    <p className="font-bold text-emerald-700">{formatPrice(format.price)}</p>
                  </div>
                  {format.disabled ? (
                    <button disabled className="mt-3 rounded-md bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-400">
                      {format.owned
                        ? language === 'ar'
                          ? 'مملوك'
                          : 'Owned'
                    ...
                    <Link
                      ...
                      className="mt-3 inline-flex rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white"
                    >
            ...
            {canRead && (
              ...
              >
                <Link
                  href={`/read/${book.id}`}
                  className="mt-5 inline-flex rounded-md border border-emerald-700 px-4 py-2 text-sm font-semibold text-emerald-700"
                >
                  {language === 'ar' ? 'فتح القارئ' : 'Open reader'}
                </Link>
              </OwnedContentGate>
            )}
          </article>
                ...
                {related.map((item) => (
                  <Link key={item.id} href={`/books/${item.id}`} className="rounded-md border border-slate-100 p-3 hover:border-emerald-200">
                    ...
                    <p className="mt-2 text-sm text-emerald-700">
                      {isFormatAvailable(item.id, 'ebook') ? formatPrice(getBookPrice(item, 'ebook') ?? item.prices.ebook) : '—'}
                    </p>
                  </Link>
                ))}
              </div>
