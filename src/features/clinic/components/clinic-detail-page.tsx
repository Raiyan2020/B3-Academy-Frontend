    ...
    <main className="min-h-screen bg-slate-50">
      <section className="bg-white">
        {clinic.image && (
          <div className="relative h-80 w-full">
            <FallbackImage src={clinic.image} alt={clinic.name} fill sizes="100vw" className="object-cover" />
          </div>
        )}
      ...
      <section className="mx-auto grid max-w-5xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_320px] lg:px-8">
        ...
        <aside className="h-fit rounded-lg border border-slate-200 bg-white p-6">
          {clinic.doctor && (
            <>
              <FallbackImage src={clinic.doctor.image} alt={clinic.doctor.name} width={80} height={80} className="h-20 w-20 rounded-full object-cover" />
