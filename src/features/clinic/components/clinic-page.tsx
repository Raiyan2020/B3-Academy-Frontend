...
export function ClinicPage() {
  ...
  const clinicsQuery = useClinics(query);
  const servicesQuery = useClinicServices();
  const clinics = useMemo(() => clinicsQuery.data ?? [], [clinicsQuery.data]);
  const services = servicesQuery.data ?? [];

  const categories = useMemo(
    () => Array.from(new Set(clinics.map((clinic) => clinic.category?.name).filter(Boolean) as string[])),
    [clinics],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return clinics.filter((clinic) => {
      const byName = clinic.name.toLowerCase().includes(term);
      const byCategory = category === 'all' || clinic.category?.name === category;
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <article key={service.id} className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                  {service.image && (
                    <div className="relative h-44 w-full">
                      <FallbackImage src={service.image} alt={service.name} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                    </div>
                  )}
                  <div className="p-5">
        ...
        {clinicsQuery.isLoading ? (
          <div className="rounded-lg border border-dashed border-slate-300 bg-white p-10 text-center text-slate-600">{language === 'ar' ? 'جارٍ التحميل...' : 'Loading...'}</div>
        ) : clinicsQuery.isError ? (
          <div className="rounded-lg border border-dashed border-red-300 bg-white p-10 text-center text-red-600">{language === 'ar' ? 'تعذّر تحميل العيادات.' : 'Failed to load clinics.'}</div>
        ) : filtered.length === 0 ? (
              ...
              <Link key={clinic.id} href={`/clinic/${clinic.id}`} className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:border-emerald-200 hover:shadow-md">
                {clinic.image && (
                  <div className="relative h-52 w-full">
                    <FallbackImage src={clinic.image} alt={clinic.name} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  </div>
                )}
                <div className="p-5">
                  {clinic.category && (
                  ...
                  <p className="mt-3 line-clamp-2 text-sm leading-6 text-slate-600">{clinic.shortDescription}</p>
                </div>
              </Link>
            ))}
          </div>
