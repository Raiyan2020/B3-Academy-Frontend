...
import { ArrowLeft, Calendar, Clock, MapPin, CheckCircle2, AlertCircle, ArrowRight } from 'lucide-react';
...
export const ClinicBookingDetail: React.FC = () => {
  ...
  const booking = user.clinicBookings?.find(c => c.id === id);
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="p-8 md:p-10">
            <div className="flex items-start justify-between mb-8">
              ...
              <span className="px-4 py-1.5 bg-[#e8f2ea] text-[#347c4c] text-sm font-bold rounded-full">
                {booking.isCompleted ? t('dash.completed') : t('consult.upcoming')}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-y-8 gap-x-12">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-sm text-slate-400 font-bold mb-0.5">{language === 'ar' ? 'الموقع' : 'Location'}</div>
          ...
          <div className="p-8 md:p-10 bg-[#f9f7f0] border-t border-slate-100">
            <div className="mb-10">
              <h2 className="text-2xl font-bold text-[#4a3e2e] mb-6">{language === 'ar' ? 'تفاصيل العيادة' : 'Clinic Details'}</h2>
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <img src="https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?q=80&w=2053&auto=format&fit=crop" alt="Clinic Interior" className="w-full h-48 object-cover" />
                <div className="p-6">
                  <h3 className="font-bold text-[#4a3e2e] text-lg mb-2">{booking.location}</h3>
                  <p className="text-slate-600 leading-relaxed mb-4">
                    {language === 'ar' 
                      ? 'عيادتنا مجهزة بأحدث التقنيات لضمان بيئة هادئة ومريحة لجميع زوارنا. يرجى الوصول قبل 15 دقيقة من موعدك.' 
                      : 'Our clinic is equipped with the latest technology to ensure a calm and comfortable environment for all our visitors. Please arrive 15 minutes before your appointment.'}
                  </p>
                  <a href={`https://maps.google.com/?q=${booking.location}`} target="_blank" rel="noopener noreferrer" className="text-[#3c7891] hover:underline font-bold inline-flex items-center gap-2">
                    <MapPin size={18} />
            ...
            <div>
              ...
              <div className={`p-6 rounded-2xl border ${user.healthAssessmentCompleted ? 'bg-emerald-50 border-emerald-100' : 'bg-[#fffaf0] border-[#feecc8]'}`}>
                <div className="flex items-start gap-6">
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 ${user.healthAssessmentCompleted ? 'bg-emerald-100 text-emerald-600' : 'bg-[#fff3d9] text-[#d97706]'}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <p className={`text-lg mb-6 leading-relaxed ${user.healthAssessmentCompleted ? 'text-emerald-700' : 'text-[#92400e]'}`}>
