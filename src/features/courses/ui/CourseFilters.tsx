...
import type { CourseCategory, CourseFilters as CourseFiltersType } from '../types/api.types';
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 border-b border-slate-200 pb-3">
        <button onClick={() => update('categoryId', 'all')} className={`rounded-full px-4 py-1.5 text-sm font-semibold ${filters.categoryId === 'all' ? 'bg-emerald-700 text-white' : 'border border-slate-200 bg-white text-slate-600'}`}>
          {isAr ? 'الكل' : 'All'}
        </button>
        {categories.map((category) => (
          <button key={category.id} onClick={() => update('categoryId', category.id)} className={`rounded-full px-4 py-1.5 text-sm font-semibold ${filters.categoryId === category.id ? 'bg-emerald-700 text-white' : 'border border-slate-200 bg-white text-slate-600'}`}>
            {category.name}
          </button>
        ))}
      </div>

      <div className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-3 lg:grid-cols-7">
        <div className="flex items-center gap-2 rounded-md border border-slate-300 px-3 py-2 md:col-span-2">
          <Search className="h-4 w-4 text-slate-400" />
          <input value={filters.search || ''} onChange={(event) => update('search', event.target.value)} placeholder={isAr ? 'بحث داخل الدورات' : 'Search courses'} className="w-full text-sm outline-none" />
        </div>
