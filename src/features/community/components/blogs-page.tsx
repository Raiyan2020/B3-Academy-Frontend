...
export const Blogs: React.FC = () => {
  const { blogs, addBlog, deleteBlog, likeBlog } = useBlogs();
  const { user } = useAuth();
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-600">Read the latest articles and insights from our experts.</p>
        </div>
      ...
      {isAdding && user?.role === 'ADMIN' && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-8">
          <h2 className="text-xl font-bold text-slate-800 mb-4">Create New Blog</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {blogs.map(blog => (
          <div key={blog.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col hover:shadow-md transition-shadow">
            {blog.imageUrl && (
              <Link to={`/community/blogs/${blog.id}`} className="block aspect-video overflow-hidden">
                <img src={blog.imageUrl} alt={blog.title[language as keyof typeof blog.title] || blog.title.en} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" referrerPolicy="no-referrer" />
              </Link>
            )}
            <div className="p-6 flex flex-col flex-grow">
              ...
              <p className="text-slate-600 line-clamp-3 mb-6 flex-grow">
                {blog.content[language as keyof typeof blog.content] || blog.content.en}
              </p>
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100">
                ...
                <div className="flex items-center gap-4 text-slate-500">
                  <button 
                    ...
                    className={`flex items-center gap-1.5 transition ${user && blog.likes.includes(user.id) ? 'text-red-500' : 'hover:text-red-500'}`}
                  >
