...
export const TheoryDetail: React.FC = () => {
  ...
  const theoryItem = getTheoryById(id);
  const inactiveTheory = getTheoryById(id, { includeInactive: true });
  const theory = theories.find((t) => t.id === id);

  if (!inactiveTheory) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-20 text-center">
        <h2 className="mb-4 text-2xl font-bold text-slate-800">{isAr ? 'النظرية غير موجودة' : 'Theory not found'}</h2>
        <Link to="/community/theories" className="text-emerald-600 hover:underline">
          {isAr ? 'العودة للنظريات' : 'Return to Theories'}
        </Link>
      </div>
    );
  }
  ...
  if (!theoryItem) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-20 text-center">
        <h2 className="mb-4 text-2xl font-bold text-slate-800">{isAr ? 'النظرية غير متاحة' : 'Theory unavailable'}</h2>
        <Link to="/community/theories" className="text-emerald-600 hover:underline">
          {isAr ? 'العودة للنظريات' : 'Return to Theories'}
        </Link>
      </div>
  ...
  const hasAccess = canAccessCommunityContent(theoryItem.metadata, accessContext);
      ...
      <div className="mx-auto max-w-2xl px-4 py-20">
        <div className="mb-8 rounded-3xl border border-slate-100 bg-white p-8 shadow-sm">
          <div className="mb-4 flex items-center gap-2 text-sm font-semibold text-amber-700">
            <Lock size={16} />
            {requiresSubscription(theoryItem.metadata) ? (isAr ? 'محتوى للمشتركين' : 'Subscriber content') : (isAr ? 'يتطلب تسجيل الدخول' : 'Sign-in required')}
          </div>
          <h1 className="text-2xl font-bold text-slate-900">{theoryItem.title[language as 'en' | 'ar'] || theoryItem.title.en}</h1>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <AccessDeniedState
            variant={requiresSubscription(theoryItem.metadata) ? 'subscription_required' : 'login_required'}
            isAr={isAr}
          />
        </div>
      </div>
  ...
  const userLiked = user ? theory.likes.includes(user.id) : false;

  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
      <button onClick={() => navigate('/community/theories')} className="mb-8 flex items-center gap-2 text-slate-500 transition hover:text-emerald-600">
        <ArrowLeft size={20} className={dir === 'rtl' ? 'rotate-180' : ''} />
        <span>{isAr ? 'العودة للنظريات' : 'Back to Theories'}</span>
      </button>

      <article className="mb-12 overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm">
        {theory.imageUrl && (
          <div className="aspect-[21/9] w-full overflow-hidden">
            <img src={theory.imageUrl} alt={localizedTitle} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
          </div>
        )}
        <div className="p-8 md:p-12">
          ...
          <div className="flex items-center gap-6 border-t border-slate-100 pt-8">
            {user ? (
              <button onClick={() => likeTheory(theory.id)} className={`flex items-center gap-2 transition ${userLiked ? 'text-red-500' : 'text-slate-500 hover:text-red-500'}`}>
                <Heart size={24} className={userLiked ? 'fill-current' : ''} />
                <span className="font-medium">{theory.likes.length}</span>
              </button>
            ) : (
            ...
            <div className="flex items-center gap-2 text-slate-500">
              ...
              <span className="font-medium">{theory.comments.length}</span>
            </div>
          </div>
        </div>
      </article>
      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-8 shadow-sm md:p-12">
        <h2 className="mb-8 text-2xl font-bold text-slate-900">{isAr ? `التعليقات (${theory.comments.length})` : `Comments (${theory.comments.length})`}</h2>
        {user ? (
              ...
              <div className="flex-grow">
                <div className="mb-1 flex items-center justify-between">
                  ...
                  {(user?.role === 'ADMIN' || user?.id === comment.userId) && (
                    <button onClick={() => deleteComment(theory.id, comment.id)} className="text-slate-400 opacity-0 transition hover:text-red-500 group-hover:opacity-100">
                      <Trash2 size={16} />
                    </button>
                  )}
                </div>
