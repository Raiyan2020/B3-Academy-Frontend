...
export function DoctorRoleGate({ children }: { children: React.ReactNode }) {
  const { user } = useAuth();
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
        <div className="max-w-md rounded-lg border border-slate-200 bg-white p-8 text-center shadow-sm">
          <h1 className="text-2xl font-bold text-slate-950">{isAr ? 'تسجيل الدخول مطلوب' : 'Sign in required'}</h1>
          <p className="mt-3 text-slate-600">{isAr ? 'يرجى تسجيل الدخول للوصول إلى بوابة الطبيب.' : 'Please sign in to access the doctor portal.'}</p>
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
        <div className="max-w-md rounded-lg border border-red-200 bg-white p-8 text-center shadow-sm">
          <h1 className="text-2xl font-bold text-slate-950">{isAr ? 'الوصول مرفوض' : 'Access denied'}</h1>
          <p className="mt-3 text-slate-600">{isAr ? 'هذه المنطقة مخصصة للأطباء فقط.' : 'This area is restricted to doctors.'}</p>
